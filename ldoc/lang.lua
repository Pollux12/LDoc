...
function Lang:start_comment(v)
   ...
   -- Check for LuaLS-style comments (triple dashes only)
   local luals_line = v:match(self.luals_comment_)

   -- Accept two-dash single-line comments that start with a recognized project-level tag
   -- e.g. "-- @module GM" or "-- @alias GAMEMODE" should be considered a doc comment header.
   -- This keeps regular two-dash comments ignored while enabling compact headers.
   local twodash_project_line
   do
      local tag = v:match('^%-%-%s*@(%w+)')
      if tag then
         -- project-level tags from doc.known_tags._project_level plus alias
         if tag == 'module' or tag == 'script' or tag == 'example' or tag == 'topic' or
             tag == 'submodule' or tag == 'classmod' or tag == 'file' or tag == 'alias' then
            twodash_project_line = v
         end
      end
   end

   return line or block or luals_line or twodash_project_line, block
end
...
function Lua:_init()
   ...
   self.luals_comment_ = '^%-%-%-[^%-]'     -- used for LuaLS-style comments (exactly 3 dashes)
   ...
   elseif t == 'iden' then
      ...
      if t == 'keyword' and v == 'function' then -- case [2]
         ...
         parser = function(tags, tok)
            -- Skip framework container assignments like PLUGIN.X = function() ... end
            if name and (name:match('^PLUGIN%.') or name:match('^MODULE%.') or
                   name:match('^ENT%.') or name:match('^SWEP%.') or
                   name:match('^TOOL%.') or name:match('^EFFECT%.') or
                   name:match('^ENTITY%.') or name:match('^VEHICLE%.')) then
               return -- Don't document these
            end
            tags:add('name', name)
            parse_lua_parameters(tags, tok)
         end
      elseif t == '{' then -- case [3]
         case = 3
         parser = function(tags, tok)
            -- Skip framework container tables like PLUGIN.commands = {}
            if name and (name:match('^PLUGIN%.') or name:match('^MODULE%.') or
                   name:match('^ENT%.') or name:match('^SWEP%.') or
                   name:match('^TOOL%.') or name:match('^EFFECT%.') or
                   name:match('^ENTITY%.') or name:match('^VEHICLE%.')) then
               return -- Don't document these tables
            end
         ...
         parser = function(tags)
            -- Filter out plugin-related variable assignments (these are not documentation items)
            if name and (name:match('^PLUGIN%.') or name:match('^MODULE%.') or
                   name:match('^ENT%.') or name:match('^SWEP%.') or
                   name:match('^TOOL%.') or name:match('^EFFECT%.') or
                   name:match('^ENTITY%.') or name:match('^VEHICLE%.') or
                   name:match('^cityrp%.')) then
               return -- Don't document these variable assignments
            end
